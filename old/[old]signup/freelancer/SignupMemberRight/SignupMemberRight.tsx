...
export const SignupMemberRight = ({ page, setPage }: Props) => {
  ...
  const [selectedProvider, setSelectedProvider] = useState<'google' | 'facebook' | 'email'>(
    'email'
  );

  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    console.log('🚀  file: SignupMemberRight.tsx:51  selectedProvider:', selectedProvider);
  ...
  const onNextStep = () => {
    const pageInputs =
      page === 1
        ? ['email', 'password', 'confirmPassword', 'checkedLegal']
        : ['name or nickname', 'last name'];

    if (page === 1 && !form.checkedLegal) {
      setFormError('Please agree to the terms and conditions');
      // formValid = false;
      return;
    } else {
